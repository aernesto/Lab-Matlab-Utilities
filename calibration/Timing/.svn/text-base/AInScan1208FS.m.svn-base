classdef AInScan1208FS < handle
    ...
    % device also has an onboard microcontroller and memory that enable
    ...
    % deterministic timing: it sets values in the device's memory that
    ...
    methods (Access = private)
        ...
        function [c, v, t, u] = waveformsFromTransfers(self)
            ...
            self.baseElementCache = runningCache;
        end
    end
...
%            dataTime = max(newData(:,3));
            ...
            context.transferredData(mglGetSecs()) = newData;
        end
    end
end